...
void get_co2effects_text(OutPeriod pd, SW_RUN *sw, LOG_INFO *LogInfo) {
    ...
    Bool fullBuffer = swFALSE;
    TimeInt co2_yrIdx = sw->ModelSim.yearIdxSpinSim;
    ...
    ForEachVegType(k) {
        (void) snprintf(
            ...
            OUTSEP,
            OUT_DIGITS,
            sw->VegProdSim.veg[k].co2_multipliers[BIO_INDEX][co2_yrIdx]
        );
        fullBuffer = sw_memccpy_inc(
    ...
    ForEachVegType(k) {
        (void) snprintf(
            ...
            OUTSEP,
            OUT_DIGITS,
            sw->VegProdSim.veg[k].co2_multipliers[WUE_INDEX][co2_yrIdx]
        );
        fullBuffer = sw_memccpy_inc(
            (void **) &writePtr, endOutstr, (void *) str, '\0', &writeSize
        );
...
void get_co2effects_mem(OutPeriod pd, SW_RUN *sw, SW_OUT_DOM *OutDom) {
    ...
    ForEachVegType(k) {
        ...
        p[iOUTIndex] =
            sw->VegProdSim.veg[k]
                .co2_multipliers[BIO_INDEX][sw->ModelSim.yearIdxSpinSim];
