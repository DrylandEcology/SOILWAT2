...
void lyrTemp_to_lyrSoil_temperature(double cor[MAX_ST_RGR][MAX_LAYERS + 1],
  ...
  int debug = 0;
    ...
    #ifdef SWDEBUG
    if (debug)
      swprintf("\nConf T : i=%i, j=%i, n=%i, avgLyrTemp[%i]=%2.2f, acc=%2.2f", i, j, n, j, avgLyrTemp[j], acc);
    #endif
	}
}
...
void lyrSoil_to_lyrTemp_temperature(unsigned int nlyrSoil, double depth_Soil[],
   double avgLyrTemp[], double endTemp, unsigned int nlyrTemp, double depth_Temp[],
   double maxTempDepth, double avgLyrTempR[]){
  ...
  int debug = 0;
	...
	for (i = 0; i < nlyrTemp; i++) {
		...
		avgLyrTempR[i + 1] = interpolation(depth_Soil2[j1], depth_Soil2[j2], avgLyrTemp2[j1], avgLyrTemp2[j2], depth_Temp[i]);

		#ifdef SWDEBUG
		if (debug)

			swprintf("\nConf T: i=%i, j1=%i, j2=%i, avgLyrTempR[%i]=%2.2f, avgLyrTemp2[%i]=%2.2f, avgLyrTemp2[%i]=%2.2f, depthT[%i]=%2.2f, depthS2[%i]=%2.2f, depthS2[%i]=%2.2f", i, j1, j2, i, avgLyrTempR[i], j1, avgLyrTemp2[j1], j2, avgLyrTemp2[j2], i, depth_Temp[i], j1, depth_Soil2[j1], j2, depth_Soil2[j2]);
	...
	#ifdef SWDEBUG
	if (debug)
		swprintf("\nConf T: avgLyrTempR[%i]=%2.2f, avgLyrTempR[%i]=%2.2f", i, avgLyrTempR[i], i+1, avgLyrTempR[i+1]);
	#endif
}
  ...
  if (nRgr + 1 >= MAX_ST_RGR) {
	...
	for (i = 0; i < nRgr + 1; i++) {
        ...
        // add from previous (x1) soil layer
				...
				j = x2;
				if (LT(SW_StRegValues->depthsR[i], depths[x2])) { // soil temperatur layer ends within x2-th soil layer
	...
	if (debug) {
		for (i = 0; i < nRgr + 1; i++) {
			swprintf("\ntl_by_sl");
				for (j = 0; j < nlyrs + 1; j++)
					swprintf("[%i,%i]=%3.2f ", i, j, SW_StRegValues->tlyrs_by_slyrs[i][j]);
		}
	}
	#endif
	...
	lyrSoil_to_lyrTemp_temperature(nlyrs, depths, avgLyrTempInit, sTconst, nRgr,
	...
	lyrSoil_to_lyrTemp(SW_StRegValues->tlyrs_by_slyrs, nlyrs, width, fc_vwc, nRgr, deltaX, SW_StRegValues->fcR);
	lyrSoil_to_lyrTemp(SW_StRegValues->tlyrs_by_slyrs, nlyrs, width, wp_vwc, nRgr, deltaX, SW_StRegValues->wpR);
	...
	if (debug) {
		for (j = 0; j < nlyrs; j++) {
			swprintf("\nConv Soil depth[%i]=%2.2f, fc=%2.2f, wp=%2.2f, bDens=%2.2f, oldT=%2.2f",
				j, depths[j], fc[j], wp[j], bDensity[j], avgLyrTempInit[j]);
		}
		...
		for (i = 0; i < nRgr + 1; i++) {
			swprintf("\nConv ST depth[%i]=%2.2f, fcR=%2.2f, wpR=%2.2f, bDensR=%2.2f, oldTR=%2.2f",
				i, SW_StRegValues->depthsR[i], SW_StRegValues->fcR[i], SW_StRegValues->wpR[i], SW_StRegValues->bDensityR[i], SW_StRegValues->oldavgLyrTempR[i+1]);
		}
	}
  #endif
}
...
unsigned int adjust_Tsoil_by_freezing_and_thawing(double oldavgLyrTemp[], double avgLyrTemp[],
	...
	/* local variables explained:
	 ...
	 sFadjusted_avgLyrTemp - if soil layer temperature was changed due to freezing/thawing
	 */
 ...
 *NOTE* Function will run if maxLyrDepth > maxDepth of the equation, but the results might be slightly off...
 ...
 nRgr - the number of regressions (1 extra value is needed for the avgLyrTempR and oldavgLyrTempR for the last layer
 sFadjusted_avgLyrTemp - if soil layer temperature was changed due to freezing/thawing; if so, then temperature of the soil temperature layers need to updated as well
...
void soil_temperature(ST_RGR_VALUES* SW_StRegValues, double *surface_max,
	...
	unsigned int nlyrs, double bmLimiter, double t1Param1, double t1Param2,
	...
	double H_gt, TimeInt year, TimeInt doy, double depths[],
  ...
  if (year == 1980 && doy < 10) {
	...
	/* local variables explained:
	 ...
	 vwcR[], avgLyrTempR[] - anything with a R at the end of the variable name stands for the interpolation of that array
	 */
	...
	if (debug) {
		swprintf("\nregression values:");
		for (i = 0; i < nRgr; i++) {
			swprintf("\nk %2d width %f depth %f vwcR %f fcR %f wpR %f oldavgLyrTempR %f bDensityR %f",
			 i, deltaX, SW_StRegValues->depthsR[i], vwcR[i], SW_StRegValues->fcR[i], SW_StRegValues->wpR[i], SW_StRegValues->oldavgLyrTempR[i], SW_StRegValues->bDensityR[i]);
		}

		swprintf("\nlayer values:");
		for (i = 0; i < nlyrs; i++) {
			swprintf("\ni %2d width %f depth %f vwc %f bDensity %f",
			i, width[i], depths[i], vwc[i], bDensity[i]);
		}
		swprintf("\n");
	}
	#endif
	...
	if (*ptr_stError) {
		LogError(LogInfo, LOGWARN, "SOILWAT2 ERROR in soil temperature module: "
	...
	if (debug) {
		...
		for (i = 0; i <= nRgr + 1; i++) {
			swprintf("\nk %d oldavgLyrTempR %f avgLyrTempR %f depth %f",
				i, SW_StRegValues->oldavgLyrTempR[i], avgLyrTempR[i], (i * deltaX)); // *(oldavgLyrTempR + i) is equivalent to writing oldavgLyrTempR[i]
		}
		swprintf("\n");
	}
	#endif
	...
	lyrTemp_to_lyrSoil_temperature(SW_StRegValues->tlyrs_by_slyrs, nRgr,
		SW_StRegValues->depthsR, avgLyrTempR, nlyrs, depths, width,
        avgLyrTemp, temperatureRangeR, temperatureRange);
	...
	if (sFadjusted_avgLyrTemp) {
		lyrSoil_to_lyrTemp_temperature(nlyrs, depths, avgLyrTemp, sTconst,
            nRgr, SW_StRegValues->depthsR, theMaxDepth, avgLyrTempR);
	}
	...
	if (debug) {
		...
		for (i = 0; i < nlyrs; i++) {
			swprintf("\ni %d oldTemp %f avgLyrTemp %f swc %f, swc_sat %f depth %f frozen %f",
				i, oldavgLyrTemp[i], avgLyrTemp[i], swc[i], swc_sat[i], depths[i], lyrFrozen[i]);
		}
