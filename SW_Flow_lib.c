...
double   declin,par1,par2,ahou,hou,azmth,solrad,
		shwave,kelvin,arads,clrsky,ftemp,
		vapor, result, dayAngle, P, gamma,
		cosZ, sinZ, cosA, sinA, stepSize,
		azmthSlope, rslope;
...
/* calculate solar declination */
	...
	declin = 0.006918 - 0.399912 * cos(dayAngle) + 0.070257 * sin(dayAngle) - 0.006758 * cos(2.*dayAngle) + 0.000907 * sin(2.*dayAngle) - 0.002697 * cos(3.*dayAngle) + 0.00148 * sin(3.*dayAngle); /* Spencer (1971): declin = solar declination [radians] */
...
/* calculate the PET using Penman (1948) */
	...
	arads = 4098. * vapor/
			((avgtemp + 237.3)*(avgtemp + 237.3)) * 5./9.;	/* Allen et al. (1998, ch.3 eqn. 13) and (2005, eqn. 5): arads used in Penman (1948), eqn. 16: arads [mmHg/F] = Delta [mmHg/C] * [C/F] = slope of e:T at T=Ta = 'Slope of the Saturation Vapor Pressure-Temperature Curve' */
	...
	par2 = (1.- reflec) * shwave * (.18 +.55 * clrsky)		/* Penman (1948), eqn. 13 [mm/day]: par2 = H = net radiant energy available at surface [mm/day] */
			   - ftemp * (.56-.092 * sqrt(humid))
			   * (.10+.90*clrsky);
	P = 101.3 * pow((293. - 0.0065 * elev) / 293., 5.26);	/* Allen et al. (1998, ch.3 eqn. 7) and (2005, eqn. 3): P [kPa] = atmospheric pressure with elev [m] */
						...
						double theMaxDepth, unsigned int nRgr )
{
	...
	*/
	...
	for( i=0; i < nRgr; i++) { // goes to nRgr, because the soil temp of the last regression layer (nRgr) is the meanAirTemp
		...
		if(toDebug) printf("k %d cs %5.4f sh %5.4f\n", i, cs, sh); 
	...
	for( i=0; i < nlyrs; i++ ) {
		...
		if(toDebug) printf("i %d %d - %d depthLow %5.4f acc %5.4f depthHigh %5.4f\n", i, x1, x2, ( (x1 + 1) * deltaX), st->depths[i], (x2 != j) ? st->depthsR[x2] : -1.0);
	...
	if(toDebug) exit(0); // terminates the program, make sure to take this out later
}
