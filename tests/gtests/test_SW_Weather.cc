...
TEST_F(WeatherFixtureTest, WeatherMonthlyInputPrioritization) {
    ...
    // SW_All.Sky.r_humidity[0], SW_All.Sky.cloudcov[0], and
    // SW_All.Sky.windspeed[0] Note: Daily interpolated values in the middle of
    ...
    EXPECT_NEAR(
        SW_All.Weather.allHist[yearIndex]->cloudcov_daily[midJanDay],
        SW_All.Sky.cloudcov[0],
        tol6
    );
    EXPECT_NEAR(
...
TEST_F(WeatherFixtureTest, WeatherDailyLOCFInputValues) {
    ...
    */
    ...
    SW_All.Weather.allHist[yearIndex]->actualVaporPressure[0] =
        actVapPressTestVal;
    ...
    for (day = 0; day < MAX_DAYS; day++) {
        EXPECT_EQ(
            SW_All.Weather.allHist[yearIndex]->cloudcov_daily[day],
            cloudCovTestVal
        );
        EXPECT_EQ(
            SW_All.Weather.allHist[yearIndex]->actualVaporPressure[day],
            actVapPressTestVal
        );
        EXPECT_EQ(
            SW_All.Weather.allHist[yearIndex]->windspeed_daily[day],
            windSpeedTestVal
        );
    }
}
