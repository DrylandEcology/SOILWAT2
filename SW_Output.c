		...
		case eSW_Day:
			...
			if(k == finalValue_dy){ // if last value to be used then write to files
				...
				if(storeAllIterations && reg_file_vals_day_iters[0] != 0){
					fprintf(SW_Output_Files.fp_dy, "%d%c%d%c%s\n", SW_Model.year, _Sep, SW_Model.doy, _Sep, reg_file_vals_day_iters);
				...
				if(storeAllIterations && soil_file_vals_day_iters[0] != 0){
					fprintf(SW_Output_Files.fp_dy_soil, "%d%c%d%c%s\n", SW_Model.year, _Sep, SW_Model.doy, _Sep, soil_file_vals_day_iters);
					memset(&soil_file_vals_day_iters[0], 0, sizeof(soil_file_vals_day_iters));
				}
			}
			break;
		...
		case eSW_Week:
			...
			if(k == finalValue_wk){
				...
				if(storeAllIterations && soil_file_vals_week_iters[0] != 0){
					fprintf(SW_Output_Files.fp_wk_soil, "%d%c%d%c%s\n", SW_Model.year, _Sep, SW_Model.week, _Sep, soil_file_vals_week_iters);
					memset(&soil_file_vals_week_iters[0], 0, sizeof(soil_file_vals_week_iters));
				}
				...
				if(storeAllIterations && reg_file_vals_week_iters[0] != 0){
					fprintf(SW_Output_Files.fp_wk, "%d%c%d%c%s\n", SW_Model.year, _Sep, SW_Model.week, _Sep, reg_file_vals_week_iters);
					memset(&reg_file_vals_week_iters[0], 0, sizeof(reg_file_vals_week_iters));
				}
			}
			break;
		...
		case eSW_Month:
			...
			if(k == finalValue_mo){
				...
				if(storeAllIterations && soil_file_vals_month_iters[0] != 0){
					fprintf(SW_Output_Files.fp_mo_soil, "%d%c%d%c%s\n", SW_Model.year, _Sep, SW_Model.month, _Sep, soil_file_vals_month_iters);
				...
				if(storeAllIterations && reg_file_vals_month_iters[0] != 0){
					fprintf(SW_Output_Files.fp_mo, "%d%c%d%c%s\n", SW_Model.year, _Sep, SW_Model.month, _Sep, reg_file_vals_month_iters);
		...
		case eSW_Year:
			...
			if(k == finalValue_yr){
				...
				if(storeAllIterations && soil_file_vals_year_iters[0] != 0){
					fprintf(SW_Output_Files.fp_yr_soil, "%d%c%s\n", SW_Model.year, _Sep, soil_file_vals_year_iters);
					memset(&soil_file_vals_year_iters[0], 0, sizeof(soil_file_vals_year_iters));
				}
				...
				if(storeAllIterations && reg_file_vals_year_iters[0] != 0){
					fprintf(SW_Output_Files.fp_yr, "%d%c%s\n", SW_Model.year, _Sep, reg_file_vals_year_iters);
					memset(&reg_file_vals_year_iters[0], 0, sizeof(reg_file_vals_year_iters));
				}
			}
			break;
...
static void get_outstrleader(TimeInt pd)
{
	...
	case eSW_Day:
		...
		#ifdef STEPWAT
			if(storeAllIterations)
				sprintf(outstr_all_iters, "%d%c%d", SW_Model.year, _Sep, SW_Model.doy);
		#endif
		break;
	case eSW_Week:
		...
		#ifdef STEPWAT
			if(storeAllIterations)
				sprintf(outstr_all_iters, "%d%c%d", SW_Model.year, _Sep,
						(SW_Model.week + 1) - tOffset);
		#endif
		break;
	case eSW_Month:
	...
	case eSW_Year:
		...
		#ifdef STEPWAT
			if(storeAllIterations)
				sprintf(outstr_all_iters, "%d", SW_Model.year);
		#endif
		break;
	}
#endif
...
static void get_estab(void)
{
	...
	for (i = 0; i < v->count; i++)
	{
			...
			if (isPartialSoilwatOutput == FALSE)
			{
				float old_val = SXW_AVG.estab_avg[Iypc(Globals.currYear,p,0)];

				SXW_AVG.estab_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.estab_avg[Iypc(Globals.currYear,p,0)], v->parms[i]->estab_doy);

				SXW_AVG.estab_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val, v->parms[i]->estab_doy, SXW_AVG.estab_avg[Iypc(Globals.currYear,p,0)]);
				if(Globals.currIter == Globals.runModelIterations){
					float std_estab = sqrt(SXW_AVG.estab_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);

					sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.estab_avg[Iypc(Globals.currYear,p,0)], _Sep, std_estab);
					strcat(outstr, str);
				}
			}
			if(storeAllIterations){
...
ForEachSoilLayer(i){
	...
	//ForEachSoilLayer(i){
		if (isPartialSoilwatOutput == FALSE)
		{
			...
			SXW_AVG.vwcbulk_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.vwcbulk_avg[Iylp(Globals.currYear,i,p,0)], val[i]);
			SXW_AVG.vwcbulk_avg[Iylp(Globals.currYear,i,p,1)] += get_running_sqr(old_val, val[i], SXW_AVG.vwcbulk_avg[Iylp(Globals.currYear,i,p,0)]);

			if(Globals.currIter == Globals.runModelIterations){
				float std_vwcbulk = sqrt(SXW_AVG.vwcbulk_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.vwcbulk_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std_vwcbulk);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
ForEachSoilLayer(i){
	if (isPartialSoilwatOutput == FALSE)
	{
		float old_val = SXW_AVG.vwcmatric_avg[Iylp(Globals.currYear,i,p,0)];

		SXW_AVG.vwcmatric_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.vwcmatric_avg[Iylp(Globals.currYear,i,p,0)], val[i]);
		SXW_AVG.vwcmatric_avg[Iylp(Globals.currYear,i,p,1)] += get_running_sqr(old_val, val[i], SXW_AVG.vwcmatric_avg[Iylp(Globals.currYear,i,p,0)]);

		if(Globals.currIter == Globals.runModelIterations){
			float std_vwcmatric = sqrt(SXW_AVG.vwcmatric_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
			sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.vwcmatric_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std_vwcmatric);
			strcat(outstr, str);
		}
	}
	if(storeAllIterations){
...
static void get_dSWAbulk(RealF swa_master[16][16][20], int i, int p){
					...
					if(crit_val < prev_crit_val){ // if true then we need to recalculate
						if(swa_master[curr_crit_rank_index][kv_veg_type][i] == 0){
							...
							dSWA_bulk[curr_crit_rank_index][kv_veg_type][i] =
								swa_master[curr_crit_rank_index][kv_veg_type][i]-swa_master[curr_crit_rank_index][prev_crit_veg_type][i];
							}
					}
    ...
    printf("%d = %f\n\n", SXW.rank_SWPcrits[3], SW_VegProd.critSoilWater[SXW.rank_SWPcrits[3]]);*/
		...
		printf("dSWAbulk shrub[1,0,%d,%d]: %f\n", i,p,SXW.dSWAbulk[Itclp(1,0,i,p)]);
		...
		printf("dSWAbulk forb[3,0,%d,%d]: %f\n", i,p,SXW.dSWAbulk[Itclp(3,0,i,p)]);
		printf("dSWAbulk forb[3,1,%d,%d]: %f\n", i,p,SXW.dSWAbulk[Itclp(3,1,i,p)]);
		...
		printf("tree[0,3,%d,%d]: %f\n\n", i,p,SXW.SWA_master[Itclp(0,3,i,p)]);

		printf("dSWAbulk tree[0,0,%d,%d]: %f\n", i,p,SXW.dSWAbulk[Itclp(0,0,i,p)]);
		printf("dSWAbulk tree[0,1,%d,%d]: %f\n", i,p,SXW.dSWAbulk[Itclp(0,1,i,p)]);
		printf("dSWAbulk tree[0,2,%d,%d]: %f\n", i,p,SXW.dSWAbulk[Itclp(0,2,i,p)]);
		printf("dSWAbulk tree[0,3,%d,%d]: %f\n\n", i,p,SXW.dSWAbulk[Itclp(0,3,i,p)]);
...
static void get_swcBulk(void)
{
	...
	ForEachSoilLayer(i)
	{
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			...
			if(Globals.currIter == Globals.runModelIterations){
				float swc_std = sqrt(SXW.swc_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW.swc_avg[Iylp(Globals.currYear,i,p,0)], _Sep, swc_std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_swaBulk(void)
{
	...
	ForEachSoilLayer(i)
	{
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val = SXW_AVG.swabulk_avg[Iylp(Globals.currYear,i,p,0)];
			...
			if(Globals.currIter == Globals.runModelIterations){
				float std = sqrt(SXW_AVG.swabulk_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.swabulk_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_swaMatric(void)
{
	...
	ForEachSoilLayer(i)
	{
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val = SXW_AVG.swamatric_avg[Iylp(Globals.currYear,i,p,0)];
			...
			if(Globals.currIter == Globals.runModelIterations){
				...
				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.swamatric_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
	...
	strcat(outstr, str);
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			...
			if(Globals.currIter == Globals.runModelIterations){
				...
				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.surfacewater_avg[Iypc(Globals.currYear,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_runoff(void)
{
	...
	strcat(outstr, str);
		...
		{
			//printf("yr, p: %d, %d\n", Globals.currYear, p);
			float old_val_total = SXW_AVG.runoff_total_avg[Iypc(Globals.currYear,p,0)];
			float old_val_surface = SXW_AVG.runoff_surface_avg[Iypc(Globals.currYear,p,0)];
			float old_val_snow = SXW_AVG.runoff_snow_avg[Iypc(Globals.currYear,p,0)];

			SXW_AVG.runoff_total_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.runoff_total_avg[Iypc(Globals.currYear,p,0)], val_totalRunoff);
			SXW_AVG.runoff_total_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_total, val_totalRunoff, SXW_AVG.runoff_total_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.runoff_surface_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.runoff_surface_avg[Iypc(Globals.currYear,p,0)], val_surfaceRunoff);
			SXW_AVG.runoff_surface_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_surface, val_surfaceRunoff, SXW_AVG.runoff_surface_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.runoff_snow_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.runoff_snow_avg[Iypc(Globals.currYear,p,0)], val_snowRunoff);
			SXW_AVG.runoff_snow_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_snow, val_snowRunoff, SXW_AVG.runoff_snow_avg[Iypc(Globals.currYear,p,0)]);
			...
			if(Globals.currIter == Globals.runModelIterations){
				float std_total = sqrt(SXW_AVG.runoff_total_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_surface = sqrt(SXW_AVG.runoff_surface_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_snow = sqrt(SXW_AVG.runoff_snow_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f", _Sep, SXW_AVG.runoff_total_avg[Iypc(Globals.currYear,p,0)], _Sep, std_total,
								_Sep, SXW_AVG.runoff_surface_avg[Iypc(Globals.currYear,p,0)], _Sep, std_surface,
								_Sep, SXW_AVG.runoff_snow_avg[Iypc(Globals.currYear,p,0)], _Sep, std_snow);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
			sprintf(str_iters, "%c%7.6f%c%7.6f%c%7.6f", _Sep, val_totalRunoff, _Sep, val_surfaceRunoff, _Sep, val_snowRunoff);
			strcat(outstr_all_iters, str_iters);
		}
...
static void get_transp(void)
{
	...
	ForEachSoilLayer(i)
	{
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			...
			SXW.transpTrees_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW.transpTrees_avg[Iylp(Globals.currYear,i,p,0)], SXW.transpTrees[Ilp(i,p)]);
			SXW.transpShrubs_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW.transpShrubs_avg[Iylp(Globals.currYear,i,p,0)], SXW.transpShrubs[Ilp(i,p)]);
			SXW.transpForbs_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW.transpForbs_avg[Iylp(Globals.currYear,i,p,0)], SXW.transpForbs[Ilp(i,p)]);
			SXW.transpGrasses_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW.transpGrasses_avg[Iylp(Globals.currYear,i,p,0)], SXW.transpGrasses[Ilp(i,p)]);

			SXW.transpTotal_avg[Iylp(Globals.currYear,i,p,1)] = get_running_sqr(old_total, SXW.transpTotal[Ilp(i,p)], SXW.transpTotal_avg[Iylp(Globals.currYear,i,p,0)]);
			SXW.transpTrees_avg[Iylp(Globals.currYear,i,p,1)] = get_running_sqr(old_tree, SXW.transpTrees[Ilp(i,p)], SXW.transpTrees_avg[Iylp(Globals.currYear,i,p,0)]);
			SXW.transpShrubs_avg[Iylp(Globals.currYear,i,p,1)] = get_running_sqr(old_shrub, SXW.transpShrubs[Ilp(i,p)], SXW.transpShrubs_avg[Iylp(Globals.currYear,i,p,0)]);
			SXW.transpForbs_avg[Iylp(Globals.currYear,i,p,1)] = get_running_sqr(old_forb, SXW.transpForbs[Ilp(i,p)], SXW.transpForbs_avg[Iylp(Globals.currYear,i,p,0)]);
			SXW.transpGrasses_avg[Iylp(Globals.currYear,i,p,1)] = get_running_sqr(old_grass, SXW.transpGrasses[Ilp(i,p)], SXW.transpGrasses_avg[Iylp(Globals.currYear,i,p,0)]);

			// if last iteration need to divide by number of iterations to get average over all iterations
			if(Globals.currIter == Globals.runModelIterations){
				float std_total = sqrt(SXW.transpTotal_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
				float std_trees = sqrt(SXW.transpTrees_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
				float std_shrubs = sqrt(SXW.transpShrubs_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
...
static void get_evapSoil(void)
{
	...
	ForEachEvapLayer(i)
	{
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val = SXW_AVG.evapsoil_avg[Iylp(Globals.currYear,i,p,0)];
			...
			if(Globals.currIter == Globals.runModelIterations){
				...
				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.evapsoil_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_evapSurface(void)
{
	...
	strcat(outstr, str);
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			...
			float old_val_shrub = SXW_AVG.evapsurface_shrub_avg[Iypc(Globals.currYear,p,0)];
			float old_val_grass = SXW_AVG.evapsurface_grass_avg[Iypc(Globals.currYear,p,0)];
			float old_val_litter = SXW_AVG.evapsurface_litter_avg[Iypc(Globals.currYear,p,0)];
			float old_val_water = SXW_AVG.evapsurface_water_avg[Iypc(Globals.currYear,p,0)];

			SXW_AVG.evapsurface_total_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.evapsurface_total_avg[Iypc(Globals.currYear,p,0)], val_tot);
			SXW_AVG.evapsurface_total_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_total, val_tot, SXW_AVG.evapsurface_total_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.evapsurface_tree_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.evapsurface_tree_avg[Iypc(Globals.currYear,p,0)], val_tree);
			SXW_AVG.evapsurface_tree_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_tree, val_tree, SXW_AVG.evapsurface_tree_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.evapsurface_forb_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.evapsurface_forb_avg[Iypc(Globals.currYear,p,0)], val_forb);
			SXW_AVG.evapsurface_forb_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_forb, val_forb, SXW_AVG.evapsurface_forb_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.evapsurface_shrub_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.evapsurface_shrub_avg[Iypc(Globals.currYear,p,0)], val_shrub);
			SXW_AVG.evapsurface_shrub_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_shrub, val_shrub, SXW_AVG.evapsurface_shrub_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.evapsurface_grass_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.evapsurface_grass_avg[Iypc(Globals.currYear,p,0)], val_grass);
			SXW_AVG.evapsurface_grass_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_grass, val_grass, SXW_AVG.evapsurface_grass_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.evapsurface_litter_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.evapsurface_litter_avg[Iypc(Globals.currYear,p,0)], val_litter);
			SXW_AVG.evapsurface_litter_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_litter, val_litter, SXW_AVG.evapsurface_litter_avg[Iypc(Globals.currYear,p,0)]);
			...
			if(Globals.currIter == Globals.runModelIterations){
				...
				float std_tree = sqrt(SXW_AVG.evapsurface_tree_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_forb = sqrt(SXW_AVG.evapsurface_forb_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_shrub = sqrt(SXW_AVG.evapsurface_shrub_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_grass = sqrt(SXW_AVG.evapsurface_grass_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_litter = sqrt(SXW_AVG.evapsurface_litter_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_water = sqrt(SXW_AVG.evapsurface_water_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f",
				 				_Sep, SXW_AVG.evapsurface_total_avg[Iypc(Globals.currYear,p,0)], _Sep, std_total,
								_Sep, SXW_AVG.evapsurface_tree_avg[Iypc(Globals.currYear,p,0)], _Sep, std_tree,
								_Sep, SXW_AVG.evapsurface_shrub_avg[Iypc(Globals.currYear,p,0)], _Sep, std_shrub,
								_Sep, SXW_AVG.evapsurface_forb_avg[Iypc(Globals.currYear,p,0)], _Sep, std_forb,
								_Sep, SXW_AVG.evapsurface_grass_avg[Iypc(Globals.currYear,p,0)], _Sep, std_grass,
								_Sep, SXW_AVG.evapsurface_litter_avg[Iypc(Globals.currYear,p,0)], _Sep, std_litter,
								_Sep, SXW_AVG.evapsurface_water_avg[Iypc(Globals.currYear,p,0)], _Sep, std_water);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_interception(void)
{
	...
	strcat(outstr, str);
			...
			if (isPartialSoilwatOutput == FALSE)
			{
				...
				SXW_AVG.interception_tree_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.interception_tree_avg[Iypc(Globals.currYear,p,0)], val_tree);
				SXW_AVG.interception_tree_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_tree, val_tree, SXW_AVG.interception_tree_avg[Iypc(Globals.currYear,p,0)]);

				SXW_AVG.interception_forb_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.interception_forb_avg[Iypc(Globals.currYear,p,0)], val_forb);
				SXW_AVG.interception_forb_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_forb, val_forb, SXW_AVG.interception_forb_avg[Iypc(Globals.currYear,p,0)]);

				SXW_AVG.interception_shrub_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.interception_shrub_avg[Iypc(Globals.currYear,p,0)], val_shrub);
				SXW_AVG.interception_shrub_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_shrub, val_shrub, SXW_AVG.interception_shrub_avg[Iypc(Globals.currYear,p,0)]);

				SXW_AVG.interception_grass_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.interception_grass_avg[Iypc(Globals.currYear,p,0)], val_grass);
				SXW_AVG.interception_grass_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_grass, val_grass, SXW_AVG.interception_grass_avg[Iypc(Globals.currYear,p,0)]);
				...
				if(Globals.currIter == Globals.runModelIterations){
					...
					sprintf(str, "%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f",
									...
									_Sep, SXW_AVG.interception_tree_avg[Iypc(Globals.currYear,p,0)], _Sep, std_tree,
									_Sep, SXW_AVG.interception_shrub_avg[Iypc(Globals.currYear,p,0)], _Sep, std_shrub,
									_Sep, SXW_AVG.interception_forb_avg[Iypc(Globals.currYear,p,0)], _Sep, std_forb,
									_Sep, SXW_AVG.interception_grass_avg[Iypc(Globals.currYear,p,0)], _Sep, std_grass,
									_Sep, SXW_AVG.interception_litter_avg[Iypc(Globals.currYear,p,0)], _Sep, std_litter);
					strcat(outstr, str);
				}
			}
			if(storeAllIterations){
	...
	strcat(outstr, str);
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			...
			if(Globals.currIter == Globals.runModelIterations){
				...
				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.soilinfilt_avg[Iypc(Globals.currYear,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_lyrdrain(void)
{
	...
	ForEachSoilLayer(i)
	{
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val = SXW_AVG.lyrdrain_avg[Iylp(Globals.currYear,i,p,0)];
			...
			if(Globals.currIter == Globals.runModelIterations){
				...
				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.lyrdrain_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_hydred(void)
{
	...
	ForEachSoilLayer(i)
	{
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val_total = SXW_AVG.hydred_total_avg[Iylp(Globals.currYear,i,p,0)];
			float old_val_tree = SXW_AVG.hydred_tree_avg[Iylp(Globals.currYear,i,p,0)];
			float old_val_forb = SXW_AVG.hydred_shrub_avg[Iylp(Globals.currYear,i,p,0)];
			float old_val_shrub = SXW_AVG.hydred_forb_avg[Iylp(Globals.currYear,i,p,0)];
			float old_val_grass = SXW_AVG.hydred_grass_avg[Iylp(Globals.currYear,i,p,0)];


			SXW_AVG.hydred_total_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.hydred_total_avg[Iylp(Globals.currYear,i,p,0)], val_total);
			SXW_AVG.hydred_tree_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.hydred_tree_avg[Iylp(Globals.currYear,i,p,0)], val_tree);
			SXW_AVG.hydred_shrub_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.hydred_shrub_avg[Iylp(Globals.currYear,i,p,0)], val_shrub);
			SXW_AVG.hydred_forb_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.hydred_forb_avg[Iylp(Globals.currYear,i,p,0)], val_grass);
			SXW_AVG.hydred_grass_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.hydred_grass_avg[Iylp(Globals.currYear,i,p,0)], val_forb);

			SXW_AVG.hydred_total_avg[Iylp(Globals.currYear,i,p,1)] += get_running_sqr(old_val_total, val_total, SXW_AVG.hydred_total_avg[Iylp(Globals.currYear,i,p,0)]);
			SXW_AVG.hydred_tree_avg[Iylp(Globals.currYear,i,p,1)] += get_running_sqr(old_val_tree, val_tree, SXW_AVG.hydred_tree_avg[Iylp(Globals.currYear,i,p,0)]);
			...
			if(Globals.currIter == Globals.runModelIterations){
				...
				float std_tree = sqrt(SXW_AVG.hydred_tree_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
				float std_forb = sqrt(SXW_AVG.hydred_forb_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
				float std_shrub = sqrt(SXW_AVG.hydred_shrub_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);
				float std_grass = sqrt(SXW_AVG.hydred_grass_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);


				sprintf(str, "%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f%c%7.6f",
								_Sep, SXW_AVG.hydred_total_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std_total,
								_Sep, SXW_AVG.hydred_tree_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std_tree,
								_Sep, SXW_AVG.hydred_shrub_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std_shrub,
								_Sep, SXW_AVG.hydred_forb_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std_forb,
								_Sep, SXW_AVG.hydred_grass_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std_grass
							);
				strcat(outstr, str);
			}
		}
...
static void get_pet(void)
{
	...
	strcat(outstr, str);
		...
		{
			//printf("yr, p: %d, %d\n", Globals.currYear, p);
			float old_val = SXW_AVG.pet_avg[Iypc(Globals.currYear,p,0)];

			SXW_AVG.pet_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.pet_avg[Iypc(Globals.currYear,p,0)], val);
			SXW_AVG.pet_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val, val, SXW_AVG.pet_avg[Iypc(Globals.currYear,p,0)]);

			//if(Globals.currYear == 1 && p == 1)
				//printf("pet_avg[%d, %d], current avg: %f, %f\n", Globals.currYear,p, val, SXW_AVG.pet_avg[Iypc(Globals.currYear,p,0)]);

			if(Globals.currIter == Globals.runModelIterations){
				float std = sqrt(SXW_AVG.pet_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.pet_avg[Iypc(Globals.currYear,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_wetdays(void)
{
	...
	ForEachSoilLayer(i){
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val = SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,0)];

			SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,0)], val);
			SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,1)] += get_running_sqr(old_val, val, SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,0)]);

			/*if(Globals.currYear == 1 && p == 1 && i == 1){
				printf("old val: %f\n", old_val);
				printf("pet_avg[%d, %d], current avg: %f, %f\n", Globals.currYear,p, val, SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,0)]);
			}*/

			if(Globals.currIter == Globals.runModelIterations){
				float std = sqrt(SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.wetday_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_snowpack(void)
{
	...
	OutPeriod pd = SW_Output[eSW_SnowPack].period;
		...
		{
			//printf("yr, p: %d, %d\n", Globals.currYear, p);
			float old_val_swe = SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,0)];
			float old_val_depth = SXW_AVG.snowpack_depth_avg[Iypc(Globals.currYear,p,0)];

			SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,0)], val_swe);
			SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_swe, val_swe, SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,0)]);

			SXW_AVG.snowpack_depth_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.snowpack_depth_avg[Iypc(Globals.currYear,p,0)], val_depth);
			SXW_AVG.snowpack_depth_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val_depth, val_depth, SXW_AVG.snowpack_depth_avg[Iypc(Globals.currYear,p,0)]);

			//if(Globals.currYear == 1 && p == 1)
				//printf("snowpack_water_eqv_avg[%d, %d], current avg: %f, %f\n", Globals.currYear,p, val_swe, SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,0)]);

			if(Globals.currIter == Globals.runModelIterations){
				float std_swe = sqrt(SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);
				float std_depth = sqrt(SXW_AVG.snowpack_depth_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f%c%7.6f%c%7.6f", _Sep, SXW_AVG.snowpack_water_eqv_avg[Iypc(Globals.currYear,p,0)], _Sep, std_swe,
								_Sep, SXW_AVG.snowpack_depth_avg[Iypc(Globals.currYear,p,0)], _Sep, std_depth);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_deepswc(void)
{
	...
	strcat(outstr, str);
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val = SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,0)];

			SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,0)] = get_running_avg(SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,0)], val);
			SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,1)] += get_running_sqr(old_val, val, SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,0)]);

			//if(Globals.currYear == 1 && p == 1)
				//printf("deepswc_avg[%d, %d], current avg: %f, %f\n", Globals.currYear,p, val, SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,0)]);

			if(Globals.currIter == Globals.runModelIterations){
				float std = sqrt(SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.deepswc_avg[Iypc(Globals.currYear,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
...
static void get_soiltemp(void)
{
	...
	ForEachSoilLayer(i){
		...
		if (isPartialSoilwatOutput == FALSE)
		{
			float old_val = SXW_AVG.soiltemp_avg[Iylp(Globals.currYear,i,p,0)];

			SXW_AVG.soiltemp_avg[Iylp(Globals.currYear,i,p,0)] = get_running_avg(SXW_AVG.soiltemp_avg[Iylp(Globals.currYear,i,p,0)], val);
			SXW_AVG.soiltemp_avg[Iylp(Globals.currYear,i,p,1)] += get_running_sqr(old_val, val, SXW_AVG.soiltemp_avg[Iylp(Globals.currYear,i,p,0)]);

			if(Globals.currIter == Globals.runModelIterations){
				float std = sqrt(SXW_AVG.soiltemp_avg[Iylp(Globals.currYear,i,p,1)] / Globals.currIter);

				sprintf(str, "%c%7.6f%c%7.6f", _Sep, SXW_AVG.soiltemp_avg[Iylp(Globals.currYear,i,p,0)], _Sep, std);
				strcat(outstr, str);
			}
		}
		if(storeAllIterations){
